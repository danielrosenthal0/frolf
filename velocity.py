...
gstreamer_str = "nvarguscamerasrc sensor-id=1 ! video/x-raw(memory:NVMM), width=1920, height=1080, format=(string)NV12, framerate=60/1 ! nvvidconv flip-method=0 ! video/x-raw, width=300, height=300, format=(string)BGRx ! videoconvert ! video/x-raw, format=(string)BGR ! appsink"
...
# hsv green 
lower_colorGreen = np.array([60, 60, 100])
